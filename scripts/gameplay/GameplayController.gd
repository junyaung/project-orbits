...
## bottom to top). 44 = DreamSky(5) + Pastel(5) + Kuiper(5) + Oort(5) +
## CrystalAurora(5) + VoidZone(5) + ForgottenRuins(5) + DarkMatterReef(5) +
## WormholeGarden(4, no transitional) = 12900m, covering 1500m to 14700m. Add
## per biome (A B C D [transition]) and bump.
const COLUMN_TILE_COUNT := 44
...
func _biome_at(m: int) -> String:
	...
	if m < 14700: return "wormhole_garden"   # 4-tile biome (no transitional yet)
	return "beyond"
